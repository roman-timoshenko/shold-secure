...
//    public Response list()  {
        return
                (villageRepository.findAll().stream().map(this::convert)
    ...
    @Path("/byplayer/{playerId}")
    @GET
    public List<VillageDto> list1(@PathParam("playerId") Integer playerId){
        return villageRepository.findByPlayerId(playerId).stream().map(this::convert).collect(Collectors.toList());
    }
...
*/
    ...
    public VillageDto convert(Village village) {
...
//                village.getBody() == null ? null : village.getBody().getId(),
    ...
    public Village convert(VillageCreateDto villageCreateDto) {
